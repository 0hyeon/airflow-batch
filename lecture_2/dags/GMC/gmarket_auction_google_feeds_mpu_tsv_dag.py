# -*- coding: utf-8 -*-
"""
G마켓/옥션 Google Feeds → S3 TSV 업로드 (쿠버네티스 풀 기반)
- header.tsv + feed_00000..00099.tsv.gz
- URL 별 매핑(expand)으로 파드 N개 동시 실행 (s3_upload_pool 슬롯으로 제어)
- 모든 태스크는 경량 파드로 오버라이드 (KubernetesExecutor + pod_override)
- 로컬 구현과 의미론 동일: 404는 태스크 실패 처리, S3 ExtraArgs 미설정, 동일 TransferConfig 적용
- 완료 후 EMR DAG 트리거
"""

import logging
from datetime import timedelta
from typing import List
import platform
from requests.utils import default_user_agent

import pendulum
import requests
from requests.adapters import HTTPAdapter
from urllib3.util.retry import Retry

from airflow.decorators import dag, task
from airflow.operators.trigger_dagrun import TriggerDagRunOperator
from airflow.providers.amazon.aws.hooks.s3 import S3Hook
from airflow.exceptions import AirflowException

from boto3.s3.transfer import TransferConfig
from botocore.config import Config as BotoConfig
import boto3

# ===================== 상수 =====================
S3_CONN_ID = "aws_conn_id"
S3_BUCKET = "gyoung0-test"
AWS_REGION = "ap-northeast-2"

# 네트워크/업로드 튜닝 (필요 시 조정)
HTTP_TIMEOUT_SEC = 300
HTTP_POOL_SIZE = 50
RETRY_TOTAL = 3

log = logging.getLogger(__name__)

# ── 경량 파드 오버라이드 (모든 태스크 공통) ──────────────────────────────
# * Airflow KubernetesExecutor에서 pod_override(V1Pod) 적용
from kubernetes.client import (
    V1Pod,
    V1ObjectMeta,
    V1PodSpec,
    V1Affinity,
    V1PodAntiAffinity,
    V1WeightedPodAffinityTerm,
    V1PodAffinityTerm,
    V1LabelSelector,
    V1LabelSelectorRequirement,
    V1Container,
    V1ResourceRequirements,
    V1EnvVar,
)

EXECUTOR_CONFIG_LITE = {
    "pod_override": V1Pod(
        api_version="v1",
        kind="Pod",
        metadata=V1ObjectMeta(labels={"app": "airflow-task-lite", "role": "lite"}),
        spec=V1PodSpec(
            restart_policy="Never",
            affinity=V1Affinity(
                pod_anti_affinity=V1PodAntiAffinity(
                    preferred_during_scheduling_ignored_during_execution=[
                        V1WeightedPodAffinityTerm(
                            weight=100,
                            pod_affinity_term=V1PodAffinityTerm(
                                label_selector=V1LabelSelector(
                                    match_expressions=[
                                        V1LabelSelectorRequirement(
                                            key="role",
                                            operator="In",
                                            values=["lite", "heavy"],
                                        )
                                    ]
                                ),
                                topology_key="kubernetes.io/hostname",
                            ),
                        )
                    ]
                )
            ),
            containers=[
                V1Container(
                    name="base",  # pod_template 컨테이너명과 동일해야 함
                    resources=V1ResourceRequirements(
                        requests={
                            "cpu": "300m",
                            "memory": "512Mi",
                            "ephemeral-storage": "1Gi",
                        },
                        limits={
                            "cpu": "1000m",
                            "memory": "1Gi",
                            "ephemeral-storage": "2Gi",
                        },
                    ),
                    env=[
                        V1EnvVar(
                            name="AIRFLOW__CORE__DAGBAG_IMPORT_TIMEOUT", value="1800"
                        ),
                    ],
                )
            ],
        ),
    )
}


# ===================== 공통 유틸 =====================


def _requests_session() -> requests.Session:
    sess = requests.Session()
    retry = Retry(
        total=RETRY_TOTAL,
        connect=RETRY_TOTAL,
        read=RETRY_TOTAL,
        backoff_factor=0.2,
        status_forcelist=[429, 500, 502, 503, 504],
        allowed_methods=["GET", "HEAD"],
    )
    adapter = HTTPAdapter(
        pool_connections=HTTP_POOL_SIZE, pool_maxsize=HTTP_POOL_SIZE, max_retries=retry
    )
    sess.mount("http://", adapter)
    sess.mount("https://", adapter)
    sess.headers.update({"User-Agent": "greenbricks"})
    return sess


# ===================== DAG =====================
@dag(
    dag_id="gmarket_auction_google_feeds_mpu_tsv_dag",
    start_date=pendulum.datetime(2025, 7, 27, tz="Asia/Seoul"),
    schedule="0 4,10,11,16,22 * * *",
    catchup=False,
    tags=["gmarket", "auction", "google", "feeds", "tsv", "gmc"],
    doc_md="G마켓/옥션 피드를 TSV 그대로 S3에 업로드(GMC 경로에 덮어쓰기) 후 EMR 처리 DAG를 트리거합니다.",
    default_args={
        "owner": "airflow",
        "retries": 2,
        "retry_delay": timedelta(minutes=2),
        "queue": "kubernetes",  # KubernetesExecutor 큐
    },
    # DAG 동시성 (클러스터/풀 상황에 맞게 조정)
    concurrency=256,
    max_active_tasks=256,
    max_active_runs=1,
)
def gmarket_google_feeds_tsv_direct_dag():
    @task(executor_config=EXECUTOR_CONFIG_LITE, pool="lite_pool")
    def get_target_info() -> dict:
        """현재 시간 기준 마켓/타겟 시각 결정."""
        now_kst = pendulum.now("Asia/Seoul")
        h = now_kst.hour
        if 11 <= h < 16:
            market = "auction"
            target_hour = "11"
        elif 4 <= h < 10:
            market = "gmarket"
            target_hour = "04"
        elif 10 <= h < 11:
            market = "gmarket"
            target_hour = "10"
        elif 16 <= h < 22:
            market = "gmarket"
            target_hour = "16"
        else:
            market = "gmarket"
            target_hour = "22"
        return {
            "market": market,
            "target_hour": target_hour,
            "target_date": now_kst.to_date_string().replace("-", ""),
        }

    @task(executor_config=EXECUTOR_CONFIG_LITE, pool="lite_pool")
    def build_urls(info: dict) -> List[str]:
        market = info["market"]
        base = f"https://im-ep.{market}.co.kr"
        hhmm = f"{int(info['target_hour']):02d}00"
        date = info["target_date"]
        header = f"{base}/google/{date}/{hhmm}/header.tsv"
        feeds = [f"{base}/google/{date}/{hhmm}/feed_{i:05d}.tsv.gz" for i in range(100)]
        urls = [header, *feeds]
        log.info(f"총 {len(urls)}개 URL 대상")
        return urls

    @task(
        executor_config=EXECUTOR_CONFIG_LITE,
        pool="s3_upload_pool",  # 업로드 병렬제어 (예: 80~120 슬롯)
        retries=2,
        retry_delay=timedelta(minutes=2),
    )
    def upload_one_to_s3(url: str, market: str) -> str:
        """단일 URL을 S3로 스트리밍 업로드 (로컬 구현과 의미론 동일)."""
        # 로컬과 동일한 Boto3 Client/TransferConfig
        s3_hook = S3Hook(aws_conn_id=S3_CONN_ID)
        creds = s3_hook.get_credentials()
        boto_cfg = BotoConfig(max_pool_connections=50, retries={"max_attempts": 3})
        transfer_cfg = TransferConfig(
            multipart_threshold=1024 * 1024 * 100,
            max_concurrency=20,
            multipart_chunksize=1024 * 1024 * 16,
            use_threads=True,
        )
        s3 = boto3.client(
            "s3",
            aws_access_key_id=creds.access_key,
            aws_secret_access_key=creds.secret_key,
            region_name=AWS_REGION,
            config=boto_cfg,
        )

        sess = _requests_session()
        try:
            file_name = url.rsplit("/", 1)[-1]  # header.tsv or feed_00000.tsv.gz

            # .tsv.gz면 그대로, 아니면 .tsv로 표준화 (로컬과 동일)
            base = file_name.rsplit(".", 1)[0]
            s3_key = (
                f"feeds/google/{market}/GMC/{file_name}"
                if url.endswith(".tsv.gz")
                else f"feeds/google/{market}/GMC/{base}.tsv"
            )

            # 404는 실패로 간주 → 태스크 실패 (로컬과 동일)
            h = sess.head(url, timeout=30)
            if h.status_code == 404:
                raise AirflowException(f"404 Not Found: {url}")
            h.raise_for_status()

            log.info(f"[GET→S3] {url} → s3://{S3_BUCKET}/{s3_key}")
            with sess.get(url, stream=True, timeout=HTTP_TIMEOUT_SEC) as r:
                r.raise_for_status()
                r.raw.decode_content = True

                # ExtraArgs 미설정 → 메타데이터 로컬과 동일(무설정)
                s3.upload_fileobj(
                    Fileobj=r.raw,
                    Bucket=S3_BUCKET,
                    Key=s3_key,
                    Config=transfer_cfg,
                )
            return f"s3://{S3_BUCKET}/{s3_key}"
        finally:
            sess.close()

    # ───────── 플로우 ─────────
    info = get_target_info()
    urls = build_urls(info)

    # URL 별 파드 매핑 → 클러스터 노드 수×슬롯 만큼 병렬
    uploaded = upload_one_to_s3.partial(
        market="{{ ti.xcom_pull(task_ids='get_target_info')['market'] }}"
    ).expand(url=urls)

    # 모두 성공 후 EMR DAG 트리거 (경량 파드 + lite_pool)
    trigger_emr_dag = TriggerDagRunOperator(
        task_id="trigger_emr_dag",
        trigger_dag_id="gmc_gmarket_auction_emr_process_and_rename_final",
        conf={"market": "{{ ti.xcom_pull(task_ids='get_target_info')['market'] }}"},
        executor_config=EXECUTOR_CONFIG_LITE,
        pool="lite_pool",
    )

    uploaded >> trigger_emr_dag


dag = gmarket_google_feeds_tsv_direct_dag()
