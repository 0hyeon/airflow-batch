# ✨ [수정] 필요한 모든 라이브러리를 정확하게 import 합니다.
...
SCRIPT_S3_KEY = "scripts/gmc_process_feeds_v3.py"
...
@dag(
    dag_id="gmc_gmarket_auction_emr_process_and_rename_final",
...
def emr_process_and_rename_final_dag():
    ...
    def submit_spark_job(cluster_id: str, dag_run: DagRun):
        ...
        whitelist_s3_path = (
            f"s3://{S3_BUCKET}/feeds/general/combined_{market}_feed.csv.gz"
        )
        s3_hook = S3Hook(aws_conn_id=AWS_CONN_ID)
        credentials = s3_hook.get_credentials()
        client = boto3.client(
