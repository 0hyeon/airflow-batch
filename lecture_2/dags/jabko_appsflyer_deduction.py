...
def run_async_processing():
    ...
    async def process_dataframes():
        ...
        paramsData = {
            "aosMediaSource": ["appier_int", "adisonofferwall_int", "cashfriends_int", "greenp_int", "buzzad_int"],
            "iosMediaSource": ["adisonofferwall_int", "cashfriends_int", "greenp_int", "buzzad_int"],
        ...
        combined_df_ws1["Event Name"] = combined_df_ws1["Event Name"].map(event_transfer).fillna("기타")
        ...
        prod_combined_df_filter["Event Name"] = prod_combined_df_filter["Event Name"].map(event_transfer).fillna("기타")
        ...
        ITET_combined_df_filter["Event Name"] = ITET_combined_df_filter["Event Name"].map(event_transfer).fillna("기타")
        ...
        CTET_combined_df_filter["Event Name"] = CTET_combined_df_filter["Event Name"].map(event_transfer).fillna("기타")
        ...
        filter_false["Event Name"] = filter_false["Event Name"].map(event_transfer).fillna("기타")
        ...
        filter_onepick["Event Name"] = filter_onepick["Event Name"].map(event_transfer).fillna("기타")
