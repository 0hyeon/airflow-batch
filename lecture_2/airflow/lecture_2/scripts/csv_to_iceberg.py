...
spark.sql("CREATE NAMESPACE IF NOT EXISTS glue.icebergtest")

yesterday = spark.sparkContext.getConf().get("spark.hadoop.yesterday")
print("✅ 전달받은 날짜:", yesterday)

csv_path = f"s3a://gyoung0-test/deduction-csv/date={yesterday}/final_attachment.csv"
...
# temp view 생성
df.createOrReplaceTempView("tmp_jobko_event_data")

# 테이블 조회
tables = spark.sql("SHOW TABLES IN glue.icebergtest").filter("tableName = 'jobko_event_data'").collect()

if len(tables) == 0:
    print("📌 테이블 없음 → 생성 진행")
    spark.sql("""
        CREATE TABLE glue.icebergtest.jobko_event_data
        USING iceberg
        PARTITIONED BY (days(partition_key))
        TBLPROPERTIES ('format-version' = '2')
        AS SELECT * FROM tmp_jobko_event_data
    """)
else:
    print("📌 테이블 존재함 → MERGE 진행")
    # 기준이 되는 unique key 지정 필요: 예시로 event_time + appsflyer_id
    spark.sql("""
        MERGE INTO glue.icebergtest.jobko_event_data t
        USING tmp_jobko_event_data s
