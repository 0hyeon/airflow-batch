...
def normalize_column_names(df):
    new_columns = [
        col_name.strip().replace(" ", "_").replace("-", "_") for col_name in df.columns
    ]
...
for file in files:
    try:
        ...
        if "retarget" in lower_name:
            df_re = (
                df.filter(col("Retargeting_Conversion_Type") == "re-engagement")
                .withColumn(
                    "event_ts", unix_timestamp("Event_Time", "yyyy-MM-dd HH:mm:ss.SSS")
                )
                .withColumn(
                    "prev2",
                    lag("event_ts", 2).over(
                        Window.partitionBy(id_col).orderBy("event_ts")
                    ),
                )
            ...
            df_prod = (
                df_re.filter(
                    (col("delta2") <= 60) & col("Media_Source").isin(media_sources)
                )
            ...
            df = df.join(
                df_prod.select("Event_Time", id_col),
                on=["Event_Time", id_col],
                how="left_anti",
            )
        ...
        df = (
            df.withColumn(
                "install_ts", unix_timestamp("Install_Time", "yyyy-MM-dd HH:mm:ss.SSS")
            )
            .withColumn(
                "event_ts", unix_timestamp("Event_Time", "yyyy-MM-dd HH:mm:ss.SSS")
            )
        ...
        df_itet = (
            df.filter(col("itet_diff") >= 24 * 1.05)
            .withColumn("Event_Name", map_event_udf("Event_Name"))
        ...
        if "Attributed_Touch_Time" in df.columns:
            df = df.withColumn(
                "touch_ts",
                unix_timestamp("Attributed_Touch_Time", "yyyy-MM-dd HH:mm:ss.SSS"),
            ).withColumn("ctit_diff", (col("install_ts") - col("touch_ts")) / 3600.0)
            df_ctit = (
                df.filter(col("ctit_diff") >= 24 * 1.05)
                .withColumn("Event_Name", map_event_udf("Event_Name"))
        ...
        if "ua" in lower_name:
            df_false = (
                df.filter(col("Is_Primary_Attribution") == "false")
                .withColumn("Event_Name", map_event_udf("Event_Name"))
        ...
        # === 5. 정상 / 원픽 ===
        df_normal = (
            df.withColumn("Event_Name", map_event_udf("Event_Name"))
...
if combined_rows:
    ...
    result_df.coalesce(1).write.option("header", True).mode("overwrite").csv(
        output_s3_path
    )
